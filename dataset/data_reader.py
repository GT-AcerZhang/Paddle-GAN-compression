    ...
    def make_reader(self):
        def reader():
            ...
            for i, fileA in enumerate(self.lines):
                ...
                imgA = Image.open(os.path.join(self.data_dir, fileA)).convert('RGB')

                if self.mode == 'TRAIN':
                    imgA = imgA.resize((self.cfgs.image_size, self.cfgs.image_size), Image.BICUBIC)

                    if self.cfgs.crop_type == 'Centor':
                        imgA = CentorCrop(imgA, self.cfgs.crop_size, self.cfgs.crop_size)
                    elif self.cfgs.crop_type == 'Random':
                        imgA = RandomCrop(imgA, self.cfgs.crop_size, self.cfgs.crop_size)
                    if self.cfgs.flip:
                        imgA = RandomHorizonFlip(imgA)
                else:
                    imgA = imgA.resize((self.cfgs.crop_size, self.cfgs.crop_size), Image.BICUBIC)
    ...
    def make_reader(self):
        def reader():
            ...
            for i, (fileA, fileB) in enumerate(zip(self.lines_A, self.lines_B)):
                fileA = fileA.strip('\n\r\t').split(' ')[0]
                fileB = fileB.strip('\n\r\t').split(' ')[0]
                self.id2name[i] = os.path.basename(fileA) + '###' + os.path.basename(fileB)

                imgA = Image.open(os.path.join(self.data_dir, fileA)).convert('RGB')
                imgB = Image.open(os.path.join(self.data_dir, fileB)).convert('RGB')

                if self.mode == 'TRAIN':
                    imgA = imgA.resize((self.cfgs.image_size, self.cfgs.image_size), Image.BICUBIC)
                    imgB = imgB.resize((self.cfgs.image_size, self.cfgs.image_size), Image.BICUBIC)

                    if self.cfgs.crop_type == 'Centor':
                        imgA = CentorCrop(imgA, self.cfgs.crop_size, self.cfgs.crop_size)
                        imgB = CentorCrop(imgB, self.cfgs.crop_size, self.cfgs.crop_size)
                    elif self.cfgs.crop_type == 'Random':
                        imgA = RandomCrop(imgA, self.cfgs.crop_size, self.cfgs.crop_size)
                        imgB = RandomCrop(imgB, self.cfgs.crop_size, self.cfgs.crop_size)
                    if self.cfgs.flip:
                        imgA = RandomHorizonFlip(imgA)
                        imgB = RandomHorizonFlip(imgB)
                else:
                    imgA = imgA.resize((self.cfgs.crop_size, self.cfgs.crop_size), Image.BICUBIC)
                    imgB = imgB.resize((self.cfgs.crop_size, self.cfgs.crop_size), Image.BICUBIC)
