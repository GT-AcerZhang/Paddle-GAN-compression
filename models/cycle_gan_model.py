...
class CycleGAN(BaseModel):
    ...
    def add_special_cfgs(parser):
        ...
        parser.add_argument('--lambda_A', type=float, default=10.0, help="weight to scale cycle loss (A->B->A)")
        parser.add_argument('--lambda_B', type=float, default=10.0, help="weight to scale cycle loss (B->A->B)")
        parser.add_argument('--lambda_identity', type=float, default=0.5, help="Weight to scale identity mapping loss")
        parser.add_argument('--pool_size', type=int, default=50, help="pool size in cyclegan")
        return parser
    ...
    def __init__(self, cfgs):
        ...
        self.netD_A = network.define_D(cfgs.output_nc, cfgs.ndf, cfgs.netD, cfgs.norm_type, cfgs.n_layer_D)
        ...
        self.optimizer_G = optimization.Optimizer(cfgs.mobile_lr, cfgs.mobile_scheduler, cfgs.step_per_epoch, cfgs.mobile_nepochs, cfgs.mobile_nepochs_decay, cfgs, parameter_list=(self.netG_A.parameters() + self.netG_B.parameters()))
        self.optimizer_D_A = optimization.Optimizer(cfgs.mobile_lr, cfgs.mobile_scheduler, cfgs.step_per_epoch, cfgs.mobile_nepochs, cfgs.mobile_nepochs_decay, cfgs, parameter_list=self.netD_A.parameters())
        self.optimizer_D_B = optimization.Optimizer(cfgs.mobile_lr, cfgs.mobile_scheduler, cfgs.step_per_epoch, cfgs.mobile_nepochs, cfgs.mobile_nepochs_decay, cfgs, parameter_list=self.netD_B.parameters())

        self.eval_dataloader_AtoB, self.name_AtoB = create_eval_data(cfgs, direction='AtoB')
        self.eval_dataloader_BtoA, self.name_BtoA = create_eval_data(cfgs, direction='BtoA')
    ...
    def backward_G(self):
        ...
        ### GAN loss D_A(G_A(A))
        self.loss_G_A = loss.gan_loss(self.cfgs.gan_loss_mode, self.netD_A(self.fake_B), True)
        ### GAN loss D_B(G_B(B))
        self.loss_G_B = loss.gan_loss(self.cfgs.gan_loss_mode, self.netD_B(self.fake_A), True)
    ...
    def evaluate_model(self, step):
        ...
        for direction in ['AtoB', 'BtoA']:
            ...
            fid = get_fid(fakes, self.inception_model, getattr(self, 'npz_%s' % direction[-1]), self.cfgs.inception_model)
            fluid.enable_imperative(place=self.cfgs.place) 
            if fid < getattr(self, 'best_fid_%s' % suffix):
                self.is_best = True
                setattr(self, 'best_fid_%s' % suffix, fid)
            print("direction: %s, fid score is: %f, best fid score is %f" % (direction, fid, getattr(self, 'best_fid_%s' % suffix)))
