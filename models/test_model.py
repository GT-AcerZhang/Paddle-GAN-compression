...
class TestModel(fluid.dygraph.Layer):
    def __init__(self, cfgs):
        ...
        self.model_names = ['G']
        self.netG = network.define_G(cfgs.input_nc, cfgs.output_nc, cfgs.ngf, cfgs.netG, cfgs.norm_type, cfgs.dropout_rate)
        self.netG.eval()
        self.cfgs = cfgs
    ...
    def profile(self, config=None):
        ...
        netG.configs = config
        with fluid.dygraph.no_grad():
            flops = dygraph_flops(netG, (self.real_A[:1]), only_conv=False, only_multiply=True)
        params = 0
