...
from model.debug_cycle_gan_model import CycleGAN
...
class Args:
    ...
    def parse(self):
        ...
        parser.add_argument('--use_gpu', type=ast.literal_eval, default=False, help='Whether to use GPU in train/test model.')
...
###############
        ...
        parser.add_argument('--save_dir', type=str, default='./output_debug', help="")
...
if __name__ == '__main__':
    with fluid.dygraph.guard(place=fluid.CUDAPlace(0)): #CPUPlace()):
        model = CycleGAN(args)
        model.setup()
        ...
        for epoch_id in range(args.epoch):
            for batch_id, (data_A, data_B) in enumerate(zip(A_loader(), B_loader())):
                ...
                #if batch_id % 10 == 0:
                if batch_id % 1 == 0:
                    print(batch_id, model.get_current_loss())
                    print(batch_id, model.get_current_lr())
                if batch_id == 3:
                    sys.exit(0)
            #if epoch_id % 10 == 0 or epoch_id == (args.epoch-1):
                #model.evaluate_mode(batch_id)
#                model.save_network(batch_id)
