...
class ResnetSupernet(BaseResnetDistiller):
    ...
    def add_special_cfgs(parser, load_pre=False):
        parser.add_argument('--supernet_lr', type=float, default=2e-4, help="Initial learning rate to train super net")
        parser.add_argument('--supernet_epoch', type=int, default=4, help="The number of epoch to train super net")
        parser.add_argument('--supernet_nepochs', type=int, default=2, help="number of epochs with the initial learning rate")
        parser.add_argument('--supernet_nepochs_decay', type=int, default=2, help="number of epochs to linearly decay learning rate to zero")
        parser.add_argument('--supernet_scheduler', type=str, default='linear', help="learning rate scheduler in train supernet")
        parser.add_argument('--supernet_student_netG', type=str, default='super_mobile_resnet_9blocks', help="Which student generator network to choose in supernet")
    ...
    def evaluate_model(self, step):
        ret = {}
        self.is_best = False
        save_dir = os.path.join(self.cfgs.save_dir, 'supernet', 'eval', str(step))
        ...
        for config_name in setting:
            ...
            ret['metric/fid_%s-mean' % config_name] = sum(getattr(self, 'fids_%s' % config_name)) / len(getattr(self, 'fids_%s' % config_name))
            ret['metric/fid_%s-best' % config_name] = getattr(self, 'best_fid_%s' % config_name)
            print("SuperNet Evalution config_name is : %s, fid score is: %f, best fid score is %f" % (config_name, fid, getattr(self, 'best_fid_%s' % config_name)))
